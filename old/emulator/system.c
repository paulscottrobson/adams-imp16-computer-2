...
void SYSWriteCharacter(int x,int y,char ch,int colour) {
    ...
    ch = ch & 0x7F;colour = colour & 7;                                                 // Mask character and colour.
    SDL_Rect rcSrc,rcTgt;
    rcSrc.w = 8;rcSrc.h = 10;rcSrc.x = (ch & 0x7F) * 8;rcSrc.y = colour*10;
...
//*******************************************************************************************************
//                           Create the 7x9 128 character font surface
//*******************************************************************************************************

#include "mcmfont.h"                                                                    // 7 x 9 Font MCM6571

static void SYSCreateFontSurface(void) {
    SDL_Rect rc;rc.w = rc.h = 1;
    SYSCreateSurface(&fontSurface,128*8,8*10);
    for (int y = 0;y < 8 * 10;y++) {
        rc.y = y;                                                                   
        int colour = y / 10;                                                            // Get colour.
        ...
        for (int x = 0;x < 128 * 8;x++) {
            if (y % 10 != 9 && _mcm6571font[x/8*9+y%10] & (0x80 >> (x % 8))) {
